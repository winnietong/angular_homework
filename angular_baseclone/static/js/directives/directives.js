...
// Display a random project in panel (4am Homework)
baseclone.directive('projectpanel', function($rootScope, ProjectFactory){
    return {
        restrict: 'A',
        templateUrl: "/static/js/views/directives/projectpanel.html",
        link: function(scope){
            $rootScope.$on('$routeChangeSuccess', function(){
                ProjectFactory.getProjects(function(response) {
                    scope.project = response[Math.floor(Math.random()*(response.length))];
                });
            });
        }
    }
